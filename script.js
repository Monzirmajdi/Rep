...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Add event listeners for newly created project cards
                    ...
                    ${project.images && project.images.length > 0 ? 
                        project.images.slice(0, Math.min(5, project.images.length)).map(img => `
                            <div class="gallery-item" style="flex: 0 0 30%; max-width:400px;min-width:250px;">
                                <img src="${img}" alt="${project.title}" class="gallery-image" style="width:100%; height:auto;border-radius:8px; opacity:0; transition: opacity 0.4s;" onload="this.style.opacity=1;">
                            </div>
                        `).join("") : 
            ...
            showMoreBtn.addEventListener("click", () => {
                imagesGrid.innerHTML = project.images.map(img => `
                    <div class="gallery-item" style="flex: 0 0 300px;">
                        <img src="${img}" alt="${project.title}" class="gallery-image" style="width:100%; border-radius:8px; opacity:0; transition: opacity 0.4s;" onload="this.style.opacity=1;">
                    </div>
                `).join("");
